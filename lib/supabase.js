...
async function uploadPrintLayout({ order, dataUrl }) {
  if (!hasSupabase()) {
    return {
      path: "",
      url: dataUrl
  ...
  const objectPath = `${order.owner_id}/${order.booth_id}/${order.id}.${imageType}`;
  const imageBuffer = Buffer.from(match[2], "base64");

  await supabaseStorageRequest(`object/${bucket}/${objectPath}`, {
    method: "PUT",
    headers: {
  ...
  const env = getEnv();
  const url = `${env.supabaseUrl}/storage/v1/object/public/${bucket}/${objectPath}`;

  try {
    await updateOrder(order.id, {
      final_image_path: objectPath,
      final_image_url: url
    });
  } catch (error) {
    console.warn("Unable to save final image URL on order:", error.message);
  }
